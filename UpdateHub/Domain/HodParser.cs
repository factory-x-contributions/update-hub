...
namespace UpdateHub.Domain
{
    public class HoDParser // for Handover Documentation *V1.2*
    {
        ...
        private static List<HandoverDocumentation> parseHoDV200(Submodel hodSubmodel)
        {
            ...
            // }
            // return documents;
            Log.Error("Handover Documentation V2.0.0 parser to be implemented ");
            return null;
        }
        ...
        private static List<HandoverDocumentation> parseHoDV120(Submodel hodSubmodel, String baseUrl, String aasIdentifierEnc)
        {
            ...
            foreach (var col in collection)
            {
                ...
                if (digitalFilePath.Substring(0, 4).Contains("http") == false)
                {
                    ...
                    var submodelIdentifierEnc = Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes(hodSubmodel.Id)).TrimEnd('=');
